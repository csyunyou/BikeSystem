...
import '../lib/js/Leaflet.Control.Custom.js'
let d3=require('d3');
// import '../lib/leaflet-heatmap.js'
let HeatmapOverlay=require('../lib/js/leaflet-heatmap.js');
let zepto=require('n-zepto')
export default{
	data(){
		return{
			map:null
	...
	mounted(){
		console.log(zepto('#leafletheatmap'))
		...
		// console.log(d3.extent(testData.data,d=>d.count))
		let vm=this;
		//this.map=L.map('leafletheatmap').setView([40.7254,-73.9757], 11);
		let baseLayer=L.tileLayer('https://api.mapbox.com/styles/v1/mapbox/dark-v9/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoieXVueW91IiwiYSI6ImNpczg2aDh0NTAwMzkyeXA3MXU3YjMxeGcifQ.z-j7gq3uS7yQ2CCjiaIp1g', {
	        maxZoom: 18,
		...
		this.map = new L.Map('leafletheatmap', {
		  center: new L.LatLng(40.7254,-73.9757),
		  zoom: 12,
		  layers: [baseLayer, heatmapLayer]
		});
		// heatmapLayer.setData(testData);
		$.ajax({
			url: 'http://localhost:3000/grouprecord/GetGroupVolumeByStation',
			type: 'GET',
		})
		.done(function(data) {
			let [minVal,maxVal]=d3.extent(data,d=>d.count);
			// console.log(minVal,maxVal);
			heatmapLayer.setData({max:maxVal,data:data});
		});
	}
}
</script>
